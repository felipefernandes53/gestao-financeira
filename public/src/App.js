...
const DEFAULT_SUBCATEGORIES = {
    [TransactionType.RECEITA]: ['Vendas de Produtos', 'Prestação de Serviços', 'Rendimentos de Investimentos', 'Outras Receitas'],
    [TransactionType.CUSTO]: ['Compra de Mercadoria (CMV)', 'Matéria-Prima', 'Embalagens', 'Fretes sobre Compras'],
    [TransactionType.DESPESA_OPERACIONAL]: ['Salários e Encargos', 'Aluguel e Condomínio', 'Marketing e Publicidade', 'Energia/Água/Internet', 'Manutenção e Reparos', 'Material de Escritório', 'Pro-labore', 'Transporte e Viagens'],
    [TransactionType.JUROS_FINANCEIROS]: ['Tarifas Bancárias', 'Juros de Empréstimos', 'Multas e Juros de Mora'],
    [TransactionType.IMPOSTOS]: ['Simples Nacional', 'ICMS', 'ISS', 'PIS', 'COFINS', 'IRPJ', 'CSLL']
};

const transactionCategories = [
    { value: TransactionType.RECEITA, label: 'Receita (+)', color: 'text-green-700 bg-green-50 dark:text-green-400 dark:bg-green-900/30', isPositive: true },
    { value: TransactionType.CUSTO, label: 'Custos (-)', color: 'text-red-700 bg-red-50 dark:text-red-400 dark:bg-red-900/30', isPositive: false },
    { value: TransactionType.DESPESA_OPERACIONAL, label: 'Desp. Operacionais (-)', color: 'text-amber-700 bg-amber-50 dark:text-amber-400 dark:bg-amber-900/30', isPositive: false },
    { value: TransactionType.JUROS_FINANCEIROS, label: 'Juros/Financeiro (-)', color: 'text-indigo-700 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30', isPositive: false },
    { value: TransactionType.IMPOSTOS, label: 'Impostos (-)', color: 'text-purple-700 bg-purple-50 dark:text-purple-400 dark:bg-purple-900/30', isPositive: false },
];
...
const calculateFinancials = (data = []) => {
    ...
    const sumByType = (type) => safeData.reduce((acc, t) => t.type === type ? acc + (Number(t.amount) || 0) : acc, 0);
    ...
    const receita = sumByType(TransactionType.RECEITA);
    const custo = sumByType(TransactionType.CUSTO);
    const despesas = sumByType(TransactionType.DESPESA_OPERACIONAL);
    const juros = sumByType(TransactionType.JUROS_FINANCEIROS);
    const impostos = sumByType(TransactionType.IMPOSTOS);
    const lucroBruto = receita - custo;
    const ebitda = lucroBruto - despesas;
    const lucroLiquido = ebitda - juros - impostos;
    const totalSaidas = custo + despesas + juros + impostos;
    
    return { receita, custo, despesas, juros, impostos, lucroBruto, ebitda, lucroLiquido, totalSaidas, fluxoCaixa: receita - totalSaidas, subcatTotals };
};

const DREView = ({ transactions, budget, isMonthly, isPrintMode }) => {
    const [expandedRows, setExpandedRows] = useState({});
    const [showPercentage, setShowPercentage] = useState(false);
    const real = useMemo(() => calculateFinancials(transactions), [transactions]);
    ...
    const DRELine = ({ label, type, valReal, valMeta, isNegative, isTotal, highlight, canExpand }) => {
        const finalReal = isNegative ? -valReal : valReal;
        ...
        return (
            <>
                <div 
                    className={`grid ${isMonthly && !showPercentage ? 'grid-cols-3' : 'grid-cols-2'} py-3 px-4 border-b border-gray-200 dark:border-slate-800 items-center ${isTotal ? (isPrintMode ? 'bg-gray-100 font-bold' : 'bg-gray-50 dark:bg-slate-800/50 font-bold') : ''} ${canExpand && !isPrintMode ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors' : ''}`}
                    onClick={() => canExpand && !isPrintMode && toggleRow(label)}
                >
                    <span className={`${isTotal ? 'text-black dark:text-white' : 'text-gray-600 dark:text-slate-300'} ${isPrintMode ? 'text-black' : ''} flex items-center gap-1`}>
                        {canExpand && !isPrintMode && (expandedRows[label] ? <LucideChevronDown size={14} /> : <LucideChevronRight size={14} />)}
                        {label}
                    </span>
        ...
        <div className={`${isPrintMode ? 'border-none' : 'bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden relative'}`}>
            {!isPrintMode && (
                <div className="absolute top-2 right-2">
                    <button onClick={() => setShowPercentage(!showPercentage)} className={`p-1.5 rounded-md transition-colors flex items-center gap-1 text-xs font-medium ${showPercentage ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'}`} title="Alternar Visualização R$ / %"><LucidePercent size={14} />{showPercentage ? 'Ver em R$' : 'Ver em %'}</button>
                </div>
            )}
            <div className={`grid ${isMonthly && !showPercentage ? 'grid-cols-3' : 'grid-cols-2'} ${isPrintMode ? 'bg-gray-200 text-black border-b border-gray-300' : 'bg-gray-100 dark:bg-slate-900 text-gray-500 dark:text-slate-400'} text-xs font-bold uppercase py-3 px-4`}>
                <div>Descrição</div><div className="text-right">{showPercentage ? 'Análise Vertical (%)' : 'Realizado (R$)'}</div>{isMonthly && !showPercentage && <div className="text-right">Variação (Meta)</div>}
            </div>
            <div>
                <DRELine label="Receita Bruta" type={TransactionType.RECEITA} valReal={real.receita} valMeta={meta[TransactionType.RECEITA]} canExpand />
                <DRELine label="(-) Custos (CMV/CSP)" type={TransactionType.CUSTO} valReal={real.custo} valMeta={meta[TransactionType.CUSTO]} isNegative canExpand />
                <DRELine label="= Lucro Bruto" valReal={real.lucroBruto} valMeta={meta.lucroBruto} isTotal />
                <DRELine label="(-) Despesas Operacionais" type={TransactionType.DESPESA_OPERACIONAL} valReal={real.despesas} valMeta={meta[TransactionType.DESPESA_OPERACIONAL]} isNegative canExpand />
                <DRELine label="= EBITDA" valReal={real.ebitda} isTotal />
                <DRELine label="(-) Juros/Financeiro" type={TransactionType.JUROS_FINANCEIROS} valReal={real.juros} valMeta={meta[TransactionType.JUROS_FINANCEIROS]} isNegative canExpand />
                <DRELine label="(-) Impostos" type={TransactionType.IMPOSTOS} valReal={real.impostos} valMeta={meta[TransactionType.IMPOSTOS]} isNegative canExpand />
                <DRELine label="= LUCRO LÍQUIDO" valReal={real.lucroLiquido} isTotal highlight />
            </div>
             {!showPercentage && <div className={`p-4 flex justify-between items-center ${isPrintMode ? 'bg-white border-t border-black' : 'bg-indigo-50 dark:bg-indigo-900/20'}`}><span className={`text-sm font-medium ${isPrintMode ? 'text-black' : 'text-indigo-800 dark:text-indigo-300'}`}>Margem Líquida</span><span className={`text-lg font-bold ${isPrintMode ? 'text-black' : 'text-indigo-900 dark:text-indigo-200'}`}>{real.receita > 0 ? `${((real.lucroLiquido / real.receita) * 100).toFixed(1)}%` : '0%'}</span></div>}
        </div>
    );
};

const BudgetPlanningView = ({ budget, subcategories, onSaveBudget, isMonthly }) => {
    ...
    const handleSubChange = (type, subName, value) => {
        ...
        setLocalSubBudget(prev => {
            ...
            const currentTypeSubs = Object.entries(newSubs)
...
const ChartsView = ({ allTransactions }) => {
    const lineData = useMemo(() => {
        ...
        return Object.keys(groups).map(key => {
            ...
            const fins = calculateFinancials(groups[key]);
            return { name: `${MONTHS[parseInt(monthStr)]}/${yearStr.slice(2)}`, Lucro: fins.lucroLiquido, Receita: fins.receita, year: parseInt(yearStr), month: parseInt(monthStr) };
...
const Sidebar = ({ isOpen, onClose, companies, currentCompany, onChangeCompany, onAddCompany, onOpenSettings, onOpenInstall, onRenameCompany }) => {
    ...
    const handleCreate = () => { if (newName.trim()) { onAddCompany(newName); setNewName(''); setIsCreating(false); onClose(); } };
...
const analytics = getAnalytics(app);!
    ...
    const resetForm = () => { setEditingTransaction(null); setFormDate(new Date().toISOString().split('T')[0]); setFormType(transactionCategories[0].value); setFormSubcat(''); setFormDesc(''); setFormAmount(''); };
    ...
    const createDefaultCompany = async (name = 'Minha Empresa') => { try { const newCompRef = doc(collection(db, `artifacts/${appId}/users/${user.uid}/companies`)); await setDoc(newCompRef, { name, createdAt: Timestamp.now() }); const batch = writeBatch(db); Object.entries(DEFAULT_SUBCATEGORIES).forEach(([type, subs]) => { subs.forEach(subName => { const ref = doc(collection(db, `artifacts/${appId}/users/${user.uid}/companies/${newCompRef.id}/subcategories`)); batch.set(ref, { type, name: subName }); }); }); await batch.commit(); if (name === 'Minha Empresa') setCurrentCompany({ id: newCompRef.id, name }); } catch (e) { console.error(e); alert("Erro ao criar empresa inicial."); } };
    const handleCreateCompany = async (name) => { if (!name || !user || !db) return; await createDefaultCompany(name); };
