...
// --- Componente de Patrimônio e Indicadores (HG Brasil) ---
const AssetsView = ({ assets, onAddAsset, onDeleteAsset }) => {
    ...
    useEffect(() => {
        const fetchMarketData = async () => {
            try {
                ...
                // 1. Moedas (AwesomeAPI)
                const resCoins = await fetch('[https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-BRL](https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-BRL)');
                const dataCoins = await resCoins.json();
                
                // 2. Índices (HG Brasil)
                // Usando modo cors-anywhere se necessário, ou fallback se falhar
                ...
                } catch(e) { console.log('HG Brasil bloqueou CORS, usando fallback seguro.'); }
                ...
                console.error("Erro API Mercado", err);
    ...
    const handleAdd = () => {
        const val = parseFloat(value.replace(/\./g, '').replace(',', '.')); // Corrige parsing BR
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                <h3 className="text-xs font-bold uppercase text-indigo-800 dark:text-indigo-300 mb-3 flex items-center gap-2"><LucideRefresh size={12}/> Mercado (HG Brasil + Awesome)</h3>
...
const ChatInterface = ({ isOpen, onClose, onAddTransaction, onAddAsset, onUpdateTransaction, onDeleteTransaction, currentCompany, transactions }) => {
    ...
    // NÃO envia no Enter (apenas Shift+Enter pula linha)
    const handleKeyDown = (e) => { 
        if (e.key === 'Enter' && !e.shiftKey) { 
            e.preventDefault(); 
            // handleSend(); // Desativado para evitar envio acidental
        } 
    }
